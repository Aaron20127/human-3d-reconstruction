...
class MPII(Dataset):
    def __init__(self,
        ...
        # key points
        self.num_joints = 19
        self.kps_map = [0, 1, 2,  3, 4, 5, 10, 11, 12, 13, 14, 15, 8, 9, 0, 0, 0, 0, 0]  # key points map lsp to smpl cocoplus key points
    ...
    def _load_data_set(self):
        clk = Clock()
        print('==> loading mpii data.'.format(self.split))
        self.images = []
        self.kp2ds = []
        ...
        if self.split == 'val':
            anno_file_path = os.path.join(self.data_path, 'validation.h5')

        with h5py.File(anno_file_path, 'r') as fp:
            self.kp2ds = np.array(fp['kp2d'])
...
if __name__ == '__main__':
    ...
    data = MPII('D:/paper/human_body_reconstruction/datasets/human_reconstruction/mpii',
                  split='train',
