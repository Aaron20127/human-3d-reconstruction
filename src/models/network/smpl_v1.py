...
class SMPLModel(Module):
    def __init__(self, device='cuda', joint_type='cocoplus',
                 model_path='D:\\paper\\human_body_reconstruction\\code\\master\\data\\neutral_smpl_with_cocoplus_reg.pkl'):
        ...
        if self.joint_type == 'cocoplus':
            self.joint_regressor = torch.from_numpy(
                np.array(params['cocoplus_regressor'].T.todense())
            ).type(torch.float64)
        else:
            self.joint_regressor = torch.from_numpy(
    ...
    def save_obj(self, verts, file_name):
    ...
    def forward(self, betas, pose, trans, simplify=False):
        ...
        return result, joints, self.faces
...
def test_gpu(gpu_id=[0]):
    ...
    for i in range(10):
        ...
        trans = torch.from_numpy(np.zeros((batch_size, 3))).type(torch.float64).to(device)
        verts, joints, faces = model(betas, pose, trans)
